...
router.get("/my-profile", isLoggedIn, (req, res, next) => {
 ...
 router.get("/favorites", isLoggedIn, async (req, res, next) => {
    ...
    try{
      const favoriteList =  (await User.findById(req.session.activeUser._id)
...
router.post("/favorites/:favoriteId/delete", isLoggedIn, async (req, res, next) => {
    const {favoriteId} = req.params
    try{
      const productoFavorito = await User.findByIdAndUpdate(req.session.activeUser._id, { $pull: { favorites: favoriteId}})
